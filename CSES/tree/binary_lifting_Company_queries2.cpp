...
class Solution{
    public:
        ll makeJump(ll node,vvll &binaryLifting,ll jump){
            for(ll i=20;i>=0;i--){
                if(((1<<i)&jump)){
        ...
        ll LCAbinarySearch(ll node1,ll node2,vvll &binaryLifting,ll high){
            ll low=0;
            ll jump=0;
            while(low<=high){
                ll mid=(low)+(high-low)/2;
                ll node1Jump=makeJump(node1,binaryLifting,mid);
                ll node2Jump=makeJump(node2,binaryLifting,mid);
            ...
            return makeJump(node1,binaryLifting,jump);
        }
        ll LCAbinaryLifting(ll node1,ll node2,vvll &binaryLifting){
            for(int i=19;i>=0;i--){
            ...
            return makeJump(node1,binaryLifting,1);
        }
        void solve(){
            ll n,queries;
            inp2(n,queries);
            ...
            while(queries--){
                ...
                if(heightNode1>heightNode2){
                    node1=makeJump(node1,binaryLifting,heightDiff);
                    ...
                    node2=makeJump(node2,binaryLifting,heightDiff);
                    if(node2==node1){
                        print1(node1+1);
                        continue;
                    }
                ...
                print1(LCAbinaryLifting(node1,node2,binaryLifting)+1);
            }
        }
};
int main()
